...
export class TestRunner {
	public static async runTest(
		...
		let timeout = test.hasOwnProperty("timeout")
			? (test as any).timeout
			: 10.0;
		let input: any = test.hasOwnProperty("input")
			? (test as TscriptInputTest).input
			: [];
		let events: any = test.hasOwnProperty("events")
			? (test as TscriptEventTest).events
			: [];

		if (
			(isBrowser === false && testCanOnlyRunInBrowser(test)) ||
